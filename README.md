...
2. cd to the dir `${IZUMI_SWAP_PERIPHERY}` and checkout to the branch you want

3. install the package listed in the `package.json` via npm

4. compile via following command

```
$ npx hardhat compile
```

5. after compile, the abi and code of the contracts can be found in files `artifacts/*.sol/*.json`

## Run test cases

1. before running test case, you should compile [izumi-swap-core](https://github.com/izumiFinance/izumi-swap-core) first.
...
2. after compile the izumi-swap-core project, copy dirs `${IZUMI_SWAP_CORE_DIR}/artifacts/*.sol` to 
...
3. running test cases via following command
```
$ npx hardhat test
```
...
1. we provide example scripts for deploy contracts, add pool, add liquidity, view liquidity, swap, prequery price.
...
3. Example to deploy `NonfungibleLiquidityManager` and `Swap` contract.
...
make sure that `nflmAddr`, `swapAddr`, `BIT`, `USDC` in the `.settings.js` is correct, `BIT` and `USDC` are 2 token address used in the script
then running,
```
