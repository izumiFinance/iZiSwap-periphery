...
abstract contract iZiSwapRouterFeeSwapType is IiZiSwapCallback {
    ...
    function swapY2XCallback(
        ...
        (address token0, address token1, uint24 feeSwapType) = dt.path.decodeFirstPool();
        uint24 fee;
        (fee,) = FeeSwapType.fromFeeSwapType(feeSwapType);
    ...
    function iZiSwapAmountSingleInternal(
        ...
        address poolAddr = iZiSwapPool(params.tokenOut, params.tokenIn, params.fee);
        uint24 feeSwapType = FeeSwapType.toFeeSwapType(params.fee, FeeSwapType.TYPE_IZISWAP);
        if (params.tokenIn < params.tokenOut) {
            // swapX2Y
            (cost, acquire) = IiZiSwapPool(poolAddr).swapX2Y(
                params.recipient, uint128(params.amount), -799999,
                abi.encode(SwapCallbackData({path: abi.encodePacked(params.tokenIn, feeSwapType, params.tokenOut), payer: payer}))
            );
        } else {
            // swapY2X
            (acquire, cost) = IiZiSwapPool(poolAddr).swapY2X(
                params.recipient, uint128(params.amount), 799999,
                abi.encode(SwapCallbackData({path: abi.encodePacked(params.tokenIn, feeSwapType, params.tokenOut), payer: payer}))
            );
        }
